...
左侧目录负责选择范围，中间只显示该目录及其子目录中的笔记。点击标题直接打开正文，不需要继续进入子目录。知识库根页面展示所有笔记，但不再重复放置目录卡片、“查看目录”或“全部笔记”折叠区。目录页原有的手写导航保留在 Markdown 源文件中，不再作为第二套可见目录展示。

顶部 **← 返回主页** 始终通向个人主页 `/`，不会因平板宽度而隐藏；头像也保留返回主页功能。面包屑中的 **知识库** 返回笔记入口 `/knowledge/`，两者不是同一个页面。
...
一个文件对应一个阅读页，标题由 `title` 生成，不必在正文重复写一级标题。文件夹构成左侧目录，笔记列表与篇数由 Markdown 自动生成，不需要修改 HTML、JavaScript 数据清单或首页链接。
...
knowledge-quartz/plugins/site-ui/         导航、笔记列表、新增/编辑入口、主题同步
knowledge-quartz/plugins/algorithm-demo/  原有算法演示
scripts/prepare-knowledge.mjs             复用主页设计变量并准备本地插件
scripts/test-knowledge-browser.py         目录范围、直接阅读、返回主页与响应式检查
...
`python3 -m http.server 8000` 在源码根目录只适合预览原生主页，不能直接渲染 Markdown 知识库。验证工作流会上传 `knowledge-ui-preview` 构建产物，解压后在其根目录启动同样的 HTTP 服务，即可连同主页一起预览 `/knowledge/`。截图产物包含知识库首页、目录列表及文章的桌面、平板和手机视图。
