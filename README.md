...
页面从标题直接开始，不显示面包屑、重复的“返回某目录”或目录/篇数统计小字。顶部 **← 返回主页** 通向个人主页 `/`，**知识库** 通向目录入口 `/knowledge/`；手机、平板和桌面均保留这些导航。头像也保留返回个人主页功能。
...
一个文件对应一个阅读页，标题由 `title` 生成，不必在正文重复写一级标题。文件夹构成左侧目录，首页目录入口和笔记列表由 Markdown 自动生成，不需要修改 HTML、JavaScript 数据清单或首页链接。
...
scripts/test-knowledge-browser.py         标题、目录入口、直接阅读、返回主页与响应式检查
